...
def test_chunks_200_frames_wan():
    ch = chunk_axis(200, 81, 8, "4n+1", [])
    assert (ch.length, ch.starts, ch.pad) == (81, (0, 60, 119), 0)


def test_chunk_frames_snapped_down_and_reported():
    notes = []
    ch = chunk_axis(300, 80, 8, "4n+1", notes)
    assert ch.length == 77 and notes == ["chunk_frames 80 -> 77 (4n+1)"]


def test_one_chunk_padded_to_rule():
    notes = []
    ch = chunk_axis(70, 81, 8, "4n+1", notes)
...
@pytest.mark.parametrize("params", [
    ...
    SplitParams(tile_mode="target size", chunk_frames=33, chunk_overlap=4, frame_rule="8n+1"),
])
...
def test_describe_lists_layout_and_adjustments():
    params = SplitParams(tile_mode="target size", target_width=1280, target_height=720,
                         chunk_frames=80, chunk_overlap=8, frame_rule="4n+1")
    ...
    assert text.splitlines() == [
        ...
        "Chunks 3 x 77f (overlap 15-16), frame rule 4n+1",
